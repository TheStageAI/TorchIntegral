...
class VarianceOptimizer:
    def __init__(self, num_iters=3000, verbose=True):
    ...
    def find_scale_vector(self, tensors, size):
        ...
        device = tensors[0]['value'].device
        best_vector = torch.ones(size, device=device)
        best_var = self.total_variance(tensors)

        if self.verbose:
            print('before scaling: ', best_var)

        for i in range(self.num_iters):
            rand_vector = torch.rand(size, device=device)
            scaled_tensors = []
            ...
            for t in tensors:
                ...
                dim = t['dim']
                shape = [1] * tensor.ndim
                shape[dim] = rand_vector.shape[0]
                scale = rand_vector.view(shape)
    ...
    # def find_scale_vector(self, tensors, size):
    #     tensors = self.choose_tensors(tensors)
    #     scale_vect = torch.nn.Parameter(
    #         torch.ones(size).to(tensors[0]['value'].device)
    #     )
    #     opt = torch.optim.Adam(
    #         [scale_vect], lr=1e-3, weight_decay=0.
    #     )
    #
    #     if self.verbose:
    #         print(
    #             'before scaling: ',
    #             self.total_variance(tensors)
    #         )
    #
    #     for i in range(self.num_iters):
    #         scaled_tensors = []
    ...
    #         print(
    #             'after scaling: ',
    #             self.total_variance(scaled_tensors)
    #         )
    #
    #     return scale_vect
